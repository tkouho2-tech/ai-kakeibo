...
def execute_expansion(username, mode="NEW", start_ym=None):
    from app import get_gspread_client, safe_gspread_call, PAYMENT_MASTER_WORKSHEET_NAME
...
def execute_variable_cost_update(username, start_ym=None, skip_backup=False):
    ...
    try:
        ...
        user_txs = [tx for tx in safe_gspread_call(get_sheet(TRANSACTIONS_WORKSHEET_NAME).get_all_records) if str(tx.get("username")).lower() == username.lower()]
        row7_pay = safe_gspread_call(ws_pay.row_values, 7)
        pay_month_indices = [(str(val).split("月")[0], idx + 1) for idx, val in enumerate(row7_pay) if idx >= 8 and "." in str(val)]
                ...
                for ym, col in pay_month_indices:
                    try:
                        ...
                        if is_cc:
                            ...
                            pers = calculate_credit_card_periods(target, str(m.get("closing_date")), str(m.get("payment_month")), str(m.get("payment_date")))
                            amt = sum(safe_money_int_cast(tx.get("amount",0)) for tx in user_txs if _normalize(tx.get("payment_method"))==_normalize(m_name) and tx.get("category")!="消費税（内税）" and pers and pers[0]["start"] <= datetime.strptime(str(tx.get("date")), "%Y-%m-%d").date() <= pers[0]["end"])
                        
                        if target_row > 0:
                            # 物理保護ロジック
                            if 78 <= target_row <= 90 and col <= 8:
                                continue
                            
                            # バッチ更新用データに追加
                            upd_pocket_list.append({'range': rowcol_to_a1(target_row, col), 'values': [[amt if amt > 0 else ""]]})
                    except: continue
                rows_79_88.append(full)
            else:
                rows_79_88.append(_dict_to_row({"大分類": "支払集計"})[1:])

        safe_gspread_call(ws_pay.update, values=rows_79_88, range_name="B79", value_input_option="USER_ENTERED")
        if upd_pocket_list:
            safe_gspread_call(ss.values_batch_update, {'valueInputOption': 'USER_ENTERED', 'data': upd_pocket_list})
...
def show_open_management_sheet():
    ...
    try:
        ss = client.open(f"{username}_支払管理")
        st.link_button("🌐 支払管理シートを開く", url=ss.url, type="primary", use_container_width=True)
...
with open('fixed_cost_expansion.py', 'w', encoding='utf-8') as f:
    f.write(content)
